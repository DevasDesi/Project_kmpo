...
class MainApp(QMainWindow):
    def __init__(self, db, user_id, role):
        ...
        self.orders_table.setColumnCount(6)  # Увеличиваем количество столбцов до 7
        self.orders_table.setHorizontalHeaderLabels(["Имя заказчика", "Номер заказа", "Товар", "Количество", "Цена", "Статус"])
    ...
    def load_orders(self):
        ...
        try:
            ...
            for row, (order_id, customer_name, product_name, quantity, total_price, status) in enumerate(orders):
    ...
    def update_orders_list(self):
        ...
        for order in orders:
            order_id, order_number, username, product_list, total_quantity, total_price, status = order
            ...
            if product_list:
                ...
                for product_details in products:
                    ...
                    self.orders_table.setItem(details_row, 0, QTableWidgetItem(""))  # Имя заказчика
                    self.orders_table.setItem(details_row, 1, QTableWidgetItem(""))  # Номер заказа
                    ...
                    self.orders_table.setItem(details_row, 5, QTableWidgetItem(""))  # Статус заказа (можно оставить пустым)

    def update_order_status(self, order_id, new_status):
        """Обновляет статус заказа в базе данных."""
        query = "UPDATE orders SET status = ? WHERE id = ?"
        try:
            # Используем метод fetch или другой подходящий для выполнения запросов
            if hasattr(self.db, 'execute'):
                self.db.execute(query, (new_status, order_id))
            elif hasattr(self.db, 'run_query'):
                self.db.run_query(query, (new_status, order_id))
            else:
                raise AttributeError("Объект базы данных не поддерживает метод выполнения запросов.")
            self.db.commit()
...
class ManageProductsDialog(QDialog):
    def __init__(self, db):
        ...
        self.products_table.setHorizontalHeaderLabels(["ID", "Название товара", "Цена (руб.)", "Количество"])
